...
export default function ProductTable({ products, getProducts, setCurrentPage }) {
	...
	const handleDeleteProduct = async () => {
		...
		try {
			const response = await fetch(`${environment.BACKEND_URL}/delete-product/${deletableProd.id}`, {
				method: 'DELETE',
				credentials: 'include'
			})
			...
			if (response.ok) {
				setCurrentPage(1)
				getProducts()
				toast.update(toastId, { render: data.message || 'Xoa success', type: 'success', isLoading: false, autoClose: 3000 })
			}
			else {
				toast.update(toastId, { render: data.message || 'Xoa Error', type: 'error', isLoading: false, autoClose: 3000 })
			}
		} catch (error) {
							...
							<tr key={index} className='text-[15px] bg-white'>
								...
								<td className='border-t border-gray-300 pl-2'>
									<div className='flex items-center gap-3 text-lg'>
										<FaEye onClick={() => navigate(`/admin/view-product/${product.id}`)} className='text-xl cursor-pointer transition-all hover:text-blue-500' />
										<LuPencil onClick={() => navigate(`/admin/edit-product/${product.id}`)} className='cursor-pointer transition-all hover:text-star' />
			...
			<div className={`${show ? 'show' : 'hidden'} overflow-y-auto overflow-x-hidden fixed top-0 bg-black/20 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full`}>
				<div className="relative p-4 w-full max-w-md h-full md:h-auto">
					<div className="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
						...
						<p className="mb-4 text-gray-500 dark:text-gray-300">Are you sure you want to delete this item <span className='font-bold'>{deletableProd?.name}</span> ?</p>
						<div className="flex justify-center items-center space-x-4">
							<button onClick={() => setShow(false)} data-modal-toggle="deleteModal" type="button" className="py-2 px-3 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-primary-300 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
								No, cancel
							</button>
							<button onClick={handleDeleteProduct} type="submit" className="py-2 px-3 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 dark:bg-red-500 dark:hover:bg-red-600 dark:focus:ring-red-900">
								Yes, sure
							</button>
						</div>
					</div>
